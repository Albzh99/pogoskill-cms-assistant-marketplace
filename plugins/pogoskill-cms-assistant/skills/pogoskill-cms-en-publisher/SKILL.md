...
5. Copy the English download CTA and Buy Box byte-for-byte from this skill's assets. Do not translate, shorten, restyle or reconstruct them. Keep the main CTA in its established position after the `How to Use PoGoskill` heading and before the `step-cont` steps; its desktop button group must remain centered. A second CTA is allowed inside FAQ only when that exact source answer explicitly introduces or recommends PoGoskill; otherwise do not add one.
